package io.github.treesitter.jtreesitter.muttrc;

import java.lang.foreign.*;

public final class TreeSitterMuttrc {
    ...
    private static final TreeSitterMuttrc INSTANCE = new TreeSitterMuttrc();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterMuttrc() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_muttrc");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-muttrc");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-muttrc");
